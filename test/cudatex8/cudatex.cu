...
texture<float_type, 2, cudaReadModeElementType> texRef2;


// Simple transformation kernel
__global__ void transformKernel(float * output_x, float_type * output_y, int width, int height, int output_width, int output_height, float theta)
{
...
int main()
{
	...
	float_type * h_data = (float_type*) malloc(width * height * sizeof(float_type));
	...
	CUDA_CALL( cudaMemcpyToArray(cuArray, 0, 0, h_data, sizeof(float_type) * size, cudaMemcpyHostToDevice) );
	...
	CUDA_CALL( cudaMemcpy(h_output_y, d_output_y, output_size * sizeof(float_type), cudaMemcpyDeviceToHost) );

	printf ("\nAfter operation\n");
	for (int i = 0 ; i < output_height; i++)
	{
		for (int j = 0 ; j < output_width; j++)
			printf ("%f ", h_output_y[i * output_width + j]);
		printf ("\n");
	}
	printf ("\n");
